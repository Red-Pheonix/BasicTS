...
class TimeSeriesForecastingInferenceRunner(BaseEpochRunner):
    """
    Runner for multivariate time series forecasting tasks.
    ...
    def build_inference_dataset(self, cfg: Dict, input_data: Union[str, list]):
        ...
        dataset = TimeSeriesFullInferenceDataset(dataset=input_data, logger=self.logger, **cfg['DATASET']['PARAM'])
        self.logger.info(f'Inference dataset length: {len(dataset)}')
    ...
    def inference(self, save_result_path: str = '') -> tuple:
        ...
        freq = self.inference_dataset.description["frequency (minutes)"]
        start_dt = self.inference_dataset.first_datetime
        
        # use tqdm to show inference progress
        pbar = tqdm(total=len(self.inference_dataset_loader), desc="Inference")

        for idx, data in enumerate(self.inference_dataset_loader):
            ...
            # ✅ take only the first predicted step
        ...
        if save_result_path:
            ...
            values = np.stack([r[1] for r in rows])  # shape: (T, N)
            ...
            df.to_csv(
                save_result_path,
                index=False,
                header=False,  # set True if you want column names
